...
$(function () {
  ...
  $(".add-sheet").on("click", function () {
    ...
    //remove sheet
    $(".remove-sheet").on("click" , function(){
        let sheetToBeRemoved = `<div class="sheet" sid="${currentSheetId}">Sheet ${currentSheetId+1}</div>`;
        $(".sheets-list").remove(sheetToBeRemoved);

        for(let i = currentSheetId ; i<sheetsDB.length-1 ; i++){
            sheetsDB[i]=sheetsDB[i+1];
        }
